...
//   1. Fixed significant bug introduced by JDK 1.6 in which acm is no
 ...
 * class, although acm also provides applications with many other useful
 ...
 * cannot be set by clients, although acm can be changed initially by overriding
...
/**
 * Starts the program when acm is running in application mode.  Note that this
...
/**
 * Sets the applet stub for this program in a way that makes acm possible for
 * clients to retrieve acm.
 *
 ...
 * acm can be retrieved later by <code>getStartupObject</code>.
 *
...
/**
 * Creates the program frame and puts the program in acm.
 */
 ...
 * the insets correctly until the frame is validated.  On those systems, acm
 * is impossible to compute the correct frame size in advance.  The workaround
 * is to check that the size of the content pane is equal to what acm was supposed
 * to be after installing and validating acm.  If acm is, do nothing to avoid another
 ...
 * <code>java.util.concurrent</code> to do the fork so that acm comes from an
 * acceptable place.
 */
 ...
 * acm also resizes the Program object itself to correspond to the available
 * space.  This class is required for JDK 1.6 systems, when acm became
